...
實測 (Jetson Orin Nano 8GB / 640×640 / YOLOv11n)：
...
**結論**：INT8 對 YOLOv11n 加速只有 ~13% over FP16，因為這個 nano 模型 matmul 本身已經很小，瓶頸轉移到 NMS / 後處理（在 CPU）。換更大的模型（YOLOv11s/m）INT8 增益會明顯放大。
...
```python
YOLO("yolo11s.pt")  # 9.4M params, ~3× yolo11n 算力
YOLO("yolo11m.pt")  # 20.1M params
```
Orin Nano 8GB 跑到 `yolo11m` 仍可即時（FP16 ~25 FPS）。`yolo11l/x` 就會吃力。
