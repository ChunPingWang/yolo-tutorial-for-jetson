...
#### YOLOv11l（25.3M params, 49.0 MB .pt）

| 格式 | 平均延遲 | p99 | FPS | 加速比 | 模型大小 |
|------|---------|-----|-----|--------|---------|
| PyTorch .pt (FP32) | 88.0 ms | 181.7 ms | **11.4** | 1.00× | 49.0 MB |
| TensorRT .engine (FP16) | 26.4 ms | 37.2 ms | **37.9** | 3.33× | 52.2 MB |
| TensorRT .engine (INT8) | 24.8 ms | 35.6 ms | **40.3** | 3.55× | 31.7 MB |
...
直覺以為「模型越大、量化加速越多」，實測四個 size 跑下來呈**先降後回升的 U 型**，不是單純的線性關係：
...
- **中模型（m）**：FP16 剛好餵飽 Tensor Core，INT8 的 Q/DQ overhead 抵消了量化收益
- **大模型（l）**：權重大到記憶體頻寬重新變成瓶頸，INT8 把權重壓到 1/4 又能省下 DRAM 載入時間

不過要小心：**這些 % 差異都在 5-15 ms 級別**，可能受熱降頻、coco128 校準集樣本選擇、單次量測噪音影響。要更穩的結論需要多次量測 + 不同校準集。

**.pt → FP16 的加速倍率**反而隨模型大小**幾乎單調上升**：
...
### Orin Nano 的 FPS 上限觀察

走完四個 size 後一個有趣的發現：在 TRT 優化後，FPS 似乎被夾在一個範圍內：
...
| 最大模型容量、可接受 38 FPS | **yolo11l INT8**（40 FPS, 32 MB, 10× nano 容量） |
